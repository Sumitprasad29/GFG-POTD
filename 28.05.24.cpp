...
{
public:
    int solution(int ind, vector<int> &cost, int w, vector<vector<int>> &dp)
    {
        if (w == 0)
        {
            return 0;
        ...
        {
            a = cost[ind] + solution(ind, cost, w - (ind + 1), dp);
        }
        b = solution(ind + 1, cost, w, dp);
